...
const PharmacyCart = () => {
  ...
  if (ordered) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-10 text-center max-w-sm w-full space-y-4">
        <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto">
          <PackageCheck size={32} className="text-green-600 dark:text-green-400" />
        </div>
        ...
        {needsRx && <p className="text-xs text-orange-600 bg-orange-50 dark:bg-orange-900/30 border border-orange-200 rounded-lg p-2">Your prescription is under review. Order will be confirmed once approved.</p>}
        <div className="flex gap-3 pt-2">
          ...
          <button onClick={() => navigate('/pharmacy')} className="flex-1 py-2.5 border border-gray-200 dark:border-gray-800 rounded-xl text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50">Continue Shopping</button>
        </div>
      </div>
    </div>
  )
        ...
        {cart.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-3">
              <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">Shopping Cart</h1>
              ...
              {cart.map(item => (
                <div key={item._id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 flex items-center gap-4">
                  <div className="w-16 h-16 bg-gray-50 dark:bg-gray-950 rounded-lg flex items-center justify-center shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.requiresPrescription && <span className="text-[10px] text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 border border-red-200 px-1.5 py-0.5 rounded">Rx Required</span>}
                    <p className="text-sm font-bold text-gray-900 dark:text-gray-100 mt-1">₹{item.price}</p>
                  </div>
              ...
              <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-gray-600 dark:text-gray-400">
                    ...
                    <span className={deliveryFee === 0 ? 'text-green-600 dark:text-green-400 font-medium' : ''}>{deliveryFee === 0 ? 'FREE' : `₹${deliveryFee}`}</span>
                  </div>
                  {deliveryFee > 0 && <p className="text-xs text-gray-400 dark:text-gray-500">Add ₹{(299 - total).toFixed(0)} more for free delivery</p>}
                  <div className="border-t pt-2 flex justify-between font-bold text-gray-900 dark:text-gray-100">
                    <span>Total</span>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Delivery Address *</label>
                  <textarea
                    rows={3}
                    value={address}
                    onChange={e => setAddress(e.target.value)}
                    placeholder="Enter your full delivery address..."
                    className="w-full border border-gray-200 dark:border-gray-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none"
                  />
                </div>

                {needsRx && (
                  <div>
                    <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Upload Prescription *</label>
                    <div
                      onClick={() => fileRef.current?.click()}
                      className="border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-xl p-4 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors"
                    >
                      {prescriptionFile ? (
                        <div className="flex items-center justify-center gap-2 text-blue-600 dark:text-blue-400">
                          <FileText size={16} />
                          <span className="text-xs font-medium truncate max-w-[140px]">{prescriptionFile.name}</span>
                          <button onClick={e => { e.stopPropagation(); setPrescriptionFile(null); setPrescriptionB64('') }}><X size={14} className="text-gray-400 dark:text-gray-500" /></button>
                        </div>
                      ) : (
                        <>
                          <Upload size={20} className="text-gray-300 mx-auto mb-1" />
                          <p className="text-xs text-gray-500">Click to upload prescription</p>
                          <p className="text-[10px] text-gray-400 dark:text-gray-500">PDF, JPG, PNG processed securely</p>
                        </>
                      )}
                    </div>
