...
const RX_STYLES = {
  'not-required': 'bg-gray-100 dark:bg-gray-800 text-gray-500',
  pending:        'bg-orange-100 text-orange-600',
  approved:       'bg-green-100 text-green-600 dark:text-green-400',
  rejected:       'bg-red-100 text-red-600 dark:text-red-400',
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-4">
        ...
        {loading ? (
          <div className="flex items-center gap-2 text-gray-400 dark:text-gray-500 py-10 justify-center"><Loader2 size={20} className="animate-spin" /> Loading orders...</div>
        ) : orders.length === 0 ? (
          ...
          orders.map(order => {
            ...
            return (
              <div key={order._id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    <p className="text-xs text-gray-400 dark:text-gray-500">Order ID</p>
                    <p className="text-sm font-mono text-gray-700 dark:text-gray-300">{order._id.slice(-10).toUpperCase()}</p>
                  </div>
                ...
                <div className="divide-y divide-gray-100">
                  {order.items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between py-2">
                      <p className="text-sm text-gray-800 dark:text-gray-200">{item.name || item.product?.name}</p>
                ...
                <div className="flex items-center justify-between pt-1 border-t border-gray-100">
                  <div className="text-xs text-gray-400 dark:text-gray-500">
                    ...
                    {order.address && <span className="ml-2 text-gray-400 dark:text-gray-500">· {order.address.slice(0, 40)}...</span>}
                  </div>
                  <p className="font-bold text-gray-900 dark:text-gray-100">₹{order.totalAmount}</p>
                </div>

                {order.prescriptionStatus === 'rejected' && (
                  <div className="flex items-center gap-2 bg-red-50 dark:bg-red-900/30 border border-red-200 rounded-lg p-2">
                    <AlertTriangle size={13} className="text-red-500 shrink-0" />
                    <p className="text-xs text-red-600 dark:text-red-400">Prescription was rejected. Please contact support or reorder with a valid prescription.</p>
                  </div>
                )}
              </div>
            )
